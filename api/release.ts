export type Build = { version: string; publishedAt: string; assetName: string; byteSize: number; digest?: string; downloadUrl: string; releaseUrl: string }
...
export function selectReleases(payload: unknown): ReleaseResponse {
  ...
  const builds = (payload as GitRelease[])
    ...
    .map((release) => ({ release, assets: (release.assets as Asset[]).filter((asset) => typeof asset.name === 'string' && /^.+\.dmg$/i.test(asset.name) && /(arm64|aarch64)/i.test(asset.name) && isPublicGithubUrl(asset.browser_download_url) && typeof asset.size === 'number' && asset.size > 0) }))
    ...
    .slice(0, MAX_BUILDS)
    .map(({ release, assets }) => {
      const asset = assets.sort((a, b) => Number(/arm64/i.test(String(b.name))) - Number(/arm64/i.test(String(a.name))))[0]
      return { version: release.tag_name as string, publishedAt: release.published_at as string, assetName: asset.name as string, byteSize: asset.size as number, ...(typeof asset.digest === 'string' ? { digest: asset.digest } : {}), downloadUrl: asset.browser_download_url as string, releaseUrl: release.html_url as string }
    })
