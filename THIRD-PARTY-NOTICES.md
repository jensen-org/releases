# Third-party notices

Jensen incorporates the third-party components listed below. Each is used under its own
license. Where a component's license grants rights broader than the Jensen End User License
Agreement in respect of that component, that license governs that component.

Regenerate this file with `scripts/third-party-notices.sh`.

## Bundled assets

| Component | Version | License |
|---|---|---|
| `Fira Code` |  | OFL-1.1 |
| `ONNX Runtime` |  | MIT |
| `CPython (python-build-standalone)` | 3.11.16 | MPL-2.0 |
| `GitHub Spec Kit` | 1.0.1 | BSD-3-Clause |

## Rust dependencies (805)

| Component | Version | License |
|---|---|---|
| `addr2line` | 0.26.1 | Apache-2.0 OR MIT |
| `adler2` | 2.0.1 | 0BSD OR Apache-2.0 OR MIT |
| `aead` | 0.5.2 | Apache-2.0 OR MIT |
| `aes` | 0.8.4 | Apache-2.0 OR MIT |
| `aes-gcm` | 0.10.3 | Apache-2.0 OR MIT |
| `ahash` | 0.8.12 | Apache-2.0 OR MIT |
| `aho-corasick` | 1.1.4 | MIT OR Unlicense |
| `alga` | 0.9.3 | Apache-2.0 |
| `alloc-no-stdlib` | 2.0.4 | BSD-3-Clause |
| `alloc-stdlib` | 0.2.4 | BSD-3-Clause |
| `allocator-api2` | 0.2.21 | Apache-2.0 OR MIT |
| `ambient-authority` | 0.0.2 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `android_system_properties` | 0.1.5 | Apache-2.0 OR MIT |
| `anyhow` | 1.0.103 | Apache-2.0 OR MIT |
| `approx` | 0.3.2 | Apache-2.0 |
| `approx` | 0.5.1 | Apache-2.0 |
| `arbitrary` | 1.4.2 | Apache-2.0 OR MIT |
| `arrayref` | 0.3.9 | BSD-2-Clause |
| `arrayvec` | 0.7.8 | Apache-2.0 OR MIT |
| `async-broadcast` | 0.7.2 | Apache-2.0 OR MIT |
| `async-channel` | 2.5.0 | Apache-2.0 OR MIT |
| `async-executor` | 1.14.0 | Apache-2.0 OR MIT |
| `async-io` | 2.6.0 | Apache-2.0 OR MIT |
| `async-lock` | 3.4.2 | Apache-2.0 OR MIT |
| `async-process` | 2.5.0 | Apache-2.0 OR MIT |
| `async-recursion` | 1.1.1 | Apache-2.0 OR MIT |
| `async-signal` | 0.2.14 | Apache-2.0 OR MIT |
| `async-task` | 4.7.1 | Apache-2.0 OR MIT |
| `async-trait` | 0.1.89 | Apache-2.0 OR MIT |
| `atk` | 0.18.2 | MIT |
| `atk-sys` | 0.18.2 | MIT |
| `atomic-waker` | 1.1.2 | Apache-2.0 OR MIT |
| `autocfg` | 1.5.1 | Apache-2.0 OR MIT |
| `base64` | 0.13.1 | Apache-2.0 OR MIT |
| `base64` | 0.21.7 | Apache-2.0 OR MIT |
| `base64` | 0.22.1 | Apache-2.0 OR MIT |
| `base64ct` | 1.8.3 | Apache-2.0 OR MIT |
| `bindgen` | 0.72.1 | BSD-3-Clause |
| `bit-set` | 0.8.0 | Apache-2.0 OR MIT |
| `bit-vec` | 0.8.0 | Apache-2.0 OR MIT |
| `bitflags` | 1.3.2 | Apache-2.0 OR MIT |
| `bitflags` | 2.13.0 | Apache-2.0 OR MIT |
| `bitmaps` | 2.1.0 | MPL-2.0+ |
| `blake3` | 1.8.5 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR CC0-1.0 |
| `block-buffer` | 0.10.4 | Apache-2.0 OR MIT |
| `block2` | 0.6.2 | MIT |
| `blocking` | 1.6.2 | Apache-2.0 OR MIT |
| `brotli` | 8.0.4 | BSD-3-Clause OR MIT |
| `brotli-decompressor` | 5.0.3 | BSD-3-Clause OR MIT |
| `bstr` | 1.12.3 | Apache-2.0 OR MIT |
| `bumpalo` | 3.20.3 | Apache-2.0 OR MIT |
| `bytemuck` | 1.25.1 | Apache-2.0 OR MIT OR Zlib |
| `byteorder` | 1.5.0 | MIT OR Unlicense |
| `bytes` | 1.12.1 | MIT |
| `cairo-rs` | 0.18.5 | MIT |
| `cairo-sys-rs` | 0.18.2 | MIT |
| `camino` | 1.2.4 | Apache-2.0 OR MIT |
| `cap-fs-ext` | 3.4.5 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `cap-primitives` | 3.4.5 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `cap-rand` | 3.4.5 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `cap-std` | 3.4.5 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `cap-time-ext` | 3.4.5 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `cargo-platform` | 0.1.9 | Apache-2.0 OR MIT |
| `cargo_metadata` | 0.19.2 | MIT |
| `cargo_toml` | 0.22.3 | Apache-2.0 OR MIT |
| `castaway` | 0.2.4 | MIT |
| `cbindgen` | 0.29.4 | MPL-2.0 |
| `cc` | 1.2.67 | Apache-2.0 OR MIT |
| `cesu8` | 1.1.0 | Apache-2.0 OR MIT |
| `cexpr` | 0.6.0 | Apache-2.0 OR MIT |
| `cfb` | 0.7.3 | MIT |
| `cfg-expr` | 0.15.8 | Apache-2.0 OR MIT |
| `cfg-if` | 1.0.4 | Apache-2.0 OR MIT |
| `cfg_aliases` | 0.1.1 | MIT |
| `cfg_aliases` | 0.2.2 | MIT |
| `chrono` | 0.4.45 | Apache-2.0 OR MIT |
| `cipher` | 0.4.4 | Apache-2.0 OR MIT |
| `clang-sys` | 1.9.1 | Apache-2.0 |
| `cobs` | 0.3.0 | Apache-2.0 OR MIT |
| `combine` | 4.6.7 | MIT |
| `compact_str` | 0.9.1 | MIT |
| `concurrent-queue` | 2.5.0 | Apache-2.0 OR MIT |
| `console` | 0.15.11 | MIT |
| `constant_time_eq` | 0.4.2 | Apache-2.0 OR CC0-1.0 OR MIT-0 |
| `cookie` | 0.18.1 | Apache-2.0 OR MIT |
| `core-foundation` | 0.10.1 | Apache-2.0 OR MIT |
| `core-foundation-sys` | 0.8.7 | Apache-2.0 OR MIT |
| `core-graphics` | 0.25.0 | Apache-2.0 OR MIT |
| `core-graphics-types` | 0.2.0 | Apache-2.0 OR MIT |
| `cpp_demangle` | 0.4.5 | Apache-2.0 OR MIT |
| `cpufeatures` | 0.2.17 | Apache-2.0 OR MIT |
| `cpufeatures` | 0.3.0 | Apache-2.0 OR MIT |
| `cranelift-assembler-x64` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-assembler-x64-meta` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-bforest` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-bitset` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-codegen` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-codegen-meta` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-codegen-shared` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-control` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-entity` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-frontend` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-isle` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-native` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `cranelift-srcgen` | 0.130.2 | Apache-2.0 WITH LLVM-exception |
| `crc` | 3.4.0 | Apache-2.0 OR MIT |
| `crc-catalog` | 2.5.0 | Apache-2.0 OR MIT |
| `crc32fast` | 1.5.0 | Apache-2.0 OR MIT |
| `crossbeam-channel` | 0.5.16 | Apache-2.0 OR MIT |
| `crossbeam-deque` | 0.8.7 | Apache-2.0 OR MIT |
| `crossbeam-epoch` | 0.9.20 | Apache-2.0 OR MIT |
| `crossbeam-utils` | 0.8.22 | Apache-2.0 OR MIT |
| `crypto-common` | 0.1.7 | Apache-2.0 OR MIT |
| `cssparser` | 0.36.0 | MPL-2.0 |
| `cssparser-macros` | 0.6.1 | MPL-2.0 |
| `csv` | 1.4.0 | MIT OR Unlicense |
| `csv-core` | 0.1.13 | MIT OR Unlicense |
| `ctor` | 0.8.0 | Apache-2.0 OR MIT |
| `ctor-proc-macro` | 0.0.7 | Apache-2.0 OR MIT |
| `ctr` | 0.9.2 | Apache-2.0 OR MIT |
| `curve25519-dalek` | 4.1.3 | BSD-3-Clause |
| `curve25519-dalek-derive` | 0.1.1 | Apache-2.0 OR MIT |
| `darling` | 0.20.11 | MIT |
| `darling` | 0.23.0 | MIT |
| `darling_core` | 0.20.11 | MIT |
| `darling_core` | 0.23.0 | MIT |
| `darling_macro` | 0.20.11 | MIT |
| `darling_macro` | 0.23.0 | MIT |
| `dary_heap` | 0.3.9 | Apache-2.0 OR MIT |
| `dashmap` | 5.5.3 | MIT |
| `dbus` | 0.9.12 | Apache-2.0 OR MIT |
| `debugid` | 0.8.0 | Apache-2.0 |
| `der` | 0.8.1 | Apache-2.0 OR MIT |
| `deranged` | 0.5.8 | Apache-2.0 OR MIT |
| `derive_arbitrary` | 1.4.2 | Apache-2.0 OR MIT |
| `derive_builder` | 0.20.2 | Apache-2.0 OR MIT |
| `derive_builder_core` | 0.20.2 | Apache-2.0 OR MIT |
| `derive_builder_macro` | 0.20.2 | Apache-2.0 OR MIT |
| `derive_more` | 2.1.1 | MIT |
| `derive_more-impl` | 2.1.1 | MIT |
| `digest` | 0.10.7 | Apache-2.0 OR MIT |
| `directories-next` | 2.0.0 | Apache-2.0 OR MIT |
| `dirs` | 6.0.0 | Apache-2.0 OR MIT |
| `dirs-sys` | 0.5.0 | Apache-2.0 OR MIT |
| `dirs-sys-next` | 0.1.2 | Apache-2.0 OR MIT |
| `dispatch2` | 0.3.1 | Apache-2.0 OR MIT OR Zlib |
| `displaydoc` | 0.2.6 | Apache-2.0 OR MIT |
| `dlopen2` | 0.8.2 | MIT |
| `dlopen2_derive` | 0.4.3 | MIT |
| `dom_query` | 0.27.0 | MIT |
| `downcast-rs` | 1.2.1 | Apache-2.0 OR MIT |
| `dpi` | 0.1.2 | Apache-2.0 OR MIT |
| `dtoa` | 1.0.11 | Apache-2.0 OR MIT |
| `dtoa-short` | 0.3.5 | MPL-2.0 |
| `dunce` | 1.0.5 | Apache-2.0 OR CC0-1.0 OR MIT-0 |
| `dyn-clone` | 1.0.20 | Apache-2.0 OR MIT |
| `ed25519` | 2.2.3 | Apache-2.0 OR MIT |
| `ed25519-dalek` | 2.2.0 | BSD-3-Clause |
| `either` | 1.16.0 | Apache-2.0 OR MIT |
| `embed-resource` | 3.0.11 | MIT |
| `embed_plist` | 1.2.2 | Apache-2.0 OR MIT |
| `encode_unicode` | 1.0.0 | Apache-2.0 OR MIT |
| `encoding_rs` | 0.8.35 | Apache-2.0 OR BSD-3-Clause OR MIT |
| `encoding_rs_io` | 0.1.7 | Apache-2.0 OR MIT |
| `endi` | 1.1.1 | MIT |
| `enumflags2` | 0.7.12 | Apache-2.0 OR MIT |
| `enumflags2_derive` | 0.7.12 | Apache-2.0 OR MIT |
| `equivalent` | 1.0.2 | Apache-2.0 OR MIT |
| `erased-serde` | 0.4.10 | Apache-2.0 OR MIT |
| `errno` | 0.3.14 | Apache-2.0 OR MIT |
| `esaxx-rs` | 0.1.10 | Apache-2.0 |
| `event-listener` | 5.4.1 | Apache-2.0 OR MIT |
| `event-listener-strategy` | 0.5.4 | Apache-2.0 OR MIT |
| `extism` | 1.30.0 | BSD-3-Clause |
| `extism-convert` | 1.30.0 | BSD-3-Clause |
| `extism-convert-macros` | 1.30.0 | BSD-3-Clause |
| `extism-manifest` | 1.30.0 | BSD-3-Clause |
| `extism-pdk` | 1.4.1 | BSD-3-Clause |
| `extism-pdk-derive` | 1.4.1 | BSD-3-Clause |
| `fallible-iterator` | 0.3.0 | Apache-2.0 OR MIT |
| `fallible-streaming-iterator` | 0.1.9 | Apache-2.0 OR MIT |
| `fastembed` | 5.17.4 | Apache-2.0 |
| `fastrand` | 2.4.1 | Apache-2.0 OR MIT |
| `fd-lock` | 4.0.4 | Apache-2.0 OR MIT |
| `fdeflate` | 0.3.7 | Apache-2.0 OR MIT |
| `fiat-crypto` | 0.2.9 | Apache-2.0 OR BSD-1-Clause OR MIT |
| `field-offset` | 0.3.6 | Apache-2.0 OR MIT |
| `filedescriptor` | 0.8.3 | MIT |
| `filetime` | 0.2.29 | Apache-2.0 OR MIT |
| `find-msvc-tools` | 0.1.9 | Apache-2.0 OR MIT |
| `fixedbitset` | 0.4.2 | Apache-2.0 OR MIT |
| `flate2` | 1.1.9 | Apache-2.0 OR MIT |
| `fnv` | 1.0.7 | Apache-2.0 OR MIT |
| `foldhash` | 0.1.5 | Zlib |
| `foldhash` | 0.2.0 | Zlib |
| `foreign-types` | 0.3.2 | Apache-2.0 OR MIT |
| `foreign-types` | 0.5.0 | Apache-2.0 OR MIT |
| `foreign-types-macros` | 0.2.3 | Apache-2.0 OR MIT |
| `foreign-types-shared` | 0.1.1 | Apache-2.0 OR MIT |
| `foreign-types-shared` | 0.3.1 | Apache-2.0 OR MIT |
| `form_urlencoded` | 1.2.2 | Apache-2.0 OR MIT |
| `fs-set-times` | 0.20.3 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `fs2` | 0.4.3 | Apache-2.0 OR MIT |
| `fsevent-sys` | 4.1.0 | MIT |
| `futures` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-channel` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-core` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-executor` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-io` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-lite` | 2.6.1 | Apache-2.0 OR MIT |
| `futures-macro` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-sink` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-task` | 0.3.32 | Apache-2.0 OR MIT |
| `futures-util` | 0.3.32 | Apache-2.0 OR MIT |
| `fxprof-processed-profile` | 0.8.1 | Apache-2.0 OR MIT |
| `gdk` | 0.18.2 | MIT |
| `gdk-pixbuf` | 0.18.5 | MIT |
| `gdk-pixbuf-sys` | 0.18.0 | MIT |
| `gdk-sys` | 0.18.2 | MIT |
| `gdkwayland-sys` | 0.18.2 | MIT |
| `gdkx11` | 0.18.2 | MIT |
| `gdkx11-sys` | 0.18.2 | MIT |
| `generic-array` | 0.14.7 | MIT |
| `gengo-language` | 0.14.3 | Apache-2.0 OR MIT |
| `getrandom` | 0.2.17 | Apache-2.0 OR MIT |
| `getrandom` | 0.3.4 | Apache-2.0 OR MIT |
| `getrandom` | 0.4.3 | Apache-2.0 OR MIT |
| `ghash` | 0.5.1 | Apache-2.0 OR MIT |
| `gimli` | 0.33.0 | Apache-2.0 OR MIT |
| `gio` | 0.18.4 | MIT |
| `gio-sys` | 0.18.1 | MIT |
| `git2` | 0.21.0 | Apache-2.0 OR MIT |
| `glib` | 0.18.5 | MIT |
| `glib-macros` | 0.18.5 | MIT |
| `glib-sys` | 0.18.1 | MIT |
| `glob` | 0.3.3 | Apache-2.0 OR MIT |
| `globset` | 0.4.18 | MIT OR Unlicense |
| `gobject-sys` | 0.18.0 | MIT |
| `graphrag-core` | 0.2.0 | MIT |
| `grep-matcher` | 0.1.8 | MIT OR Unlicense |
| `grep-regex` | 0.1.14 | MIT OR Unlicense |
| `grep-searcher` | 0.1.16 | MIT OR Unlicense |
| `gtk` | 0.18.2 | MIT |
| `gtk-sys` | 0.18.2 | MIT |
| `gtk3-macros` | 0.18.2 | MIT |
| `hashbrown` | 0.12.3 | Apache-2.0 OR MIT |
| `hashbrown` | 0.14.5 | Apache-2.0 OR MIT |
| `hashbrown` | 0.15.5 | Apache-2.0 OR MIT |
| `hashbrown` | 0.16.1 | Apache-2.0 OR MIT |
| `hashbrown` | 0.17.1 | Apache-2.0 OR MIT |
| `hashlink` | 0.9.1 | Apache-2.0 OR MIT |
| `heck` | 0.4.1 | Apache-2.0 OR MIT |
| `heck` | 0.5.0 | Apache-2.0 OR MIT |
| `hermit-abi` | 0.5.2 | Apache-2.0 OR MIT |
| `hex` | 0.4.3 | Apache-2.0 OR MIT |
| `hf-hub` | 0.5.0 | Apache-2.0 |
| `hkdf` | 0.12.4 | Apache-2.0 OR MIT |
| `hmac` | 0.12.1 | Apache-2.0 OR MIT |
| `hmac-sha256` | 1.1.14 | ISC |
| `html5ever` | 0.38.0 | Apache-2.0 OR MIT |
| `http` | 1.4.2 | Apache-2.0 OR MIT |
| `http-body` | 1.0.1 | MIT |
| `http-body-util` | 0.1.3 | MIT |
| `http-range` | 0.1.5 | MIT |
| `httparse` | 1.10.1 | Apache-2.0 OR MIT |
| `hyper` | 1.10.1 | MIT |
| `hyper-util` | 0.1.20 | MIT |
| `iana-time-zone` | 0.1.65 | Apache-2.0 OR MIT |
| `iana-time-zone-haiku` | 0.1.2 | Apache-2.0 OR MIT |
| `ico` | 0.5.0 | MIT |
| `icu_collections` | 2.2.0 | Unicode-3.0 |
| `icu_locale_core` | 2.2.0 | Unicode-3.0 |
| `icu_normalizer` | 2.2.0 | Unicode-3.0 |
| `icu_normalizer_data` | 2.2.0 | Unicode-3.0 |
| `icu_properties` | 2.2.0 | Unicode-3.0 |
| `icu_properties_data` | 2.2.0 | Unicode-3.0 |
| `icu_provider` | 2.2.0 | Unicode-3.0 |
| `id-arena` | 2.3.0 | Apache-2.0 OR MIT |
| `ident_case` | 1.0.1 | Apache-2.0 OR MIT |
| `idna` | 1.1.0 | Apache-2.0 OR MIT |
| `idna_adapter` | 1.2.2 | Apache-2.0 OR MIT |
| `ignore` | 0.4.28 | MIT OR Unlicense |
| `im-rc` | 15.1.0 | MPL-2.0+ |
| `indexmap` | 1.9.3 | Apache-2.0 OR MIT |
| `indexmap` | 2.14.0 | Apache-2.0 OR MIT |
| `indicatif` | 0.17.11 | MIT |
| `infer` | 0.19.0 | MIT |
| `inotify` | 0.9.6 | ISC |
| `inotify-sys` | 0.1.8 | ISC |
| `inout` | 0.1.4 | Apache-2.0 OR MIT |
| `io-extras` | 0.18.4 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `io-lifetimes` | 2.0.4 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `ipnet` | 2.12.0 | Apache-2.0 OR MIT |
| `is-docker` | 0.2.0 | MIT |
| `is-wsl` | 0.4.0 | MIT |
| `itertools` | 0.13.0 | Apache-2.0 OR MIT |
| `itertools` | 0.14.0 | Apache-2.0 OR MIT |
| `itoa` | 1.0.18 | Apache-2.0 OR MIT |
| `ittapi` | 0.4.0 | BSD-3-Clause OR GPL-2.0-only |
| `ittapi-sys` | 0.4.0 | BSD-3-Clause OR GPL-2.0-only |
| `javascriptcore-rs` | 1.1.2 | MIT |
| `javascriptcore-rs-sys` | 1.1.1 | MIT |
| `jni` | 0.21.1 | Apache-2.0 OR MIT |
| `jni-sys` | 0.3.1 | Apache-2.0 OR MIT |
| `jni-sys` | 0.4.1 | Apache-2.0 OR MIT |
| `jni-sys-macros` | 0.4.1 | Apache-2.0 OR MIT |
| `jobserver` | 0.1.35 | Apache-2.0 OR MIT |
| `js-sys` | 0.3.103 | Apache-2.0 OR MIT |
| `json` | 0.12.4 | Apache-2.0 OR MIT |
| `json-patch` | 3.0.1 | Apache-2.0 OR MIT |
| `jsonfixer` | 0.1.0 | MIT |
| `jsonptr` | 0.6.3 | Apache-2.0 OR MIT |
| `keyboard-types` | 0.7.0 | Apache-2.0 OR MIT |
| `kqueue` | 1.2.0 | MIT |
| `kqueue-sys` | 1.1.2 | MIT |
| `lazy_static` | 1.5.0 | Apache-2.0 OR MIT |
| `leb128` | 0.2.7 | Apache-2.0 OR MIT |
| `leb128fmt` | 0.1.0 | Apache-2.0 OR MIT |
| `libc` | 0.2.186 | Apache-2.0 OR MIT |
| `libdbus-sys` | 0.2.7 | Apache-2.0 OR MIT |
| `libgit2-sys` | 0.18.5+1.9.4 | Apache-2.0 OR MIT |
| `libloading` | 0.8.9 | ISC |
| `libm` | 0.2.16 | MIT |
| `libredox` | 0.1.18 | MIT |
| `libsqlite3-sys` | 0.30.1 | MIT |
| `libz-sys` | 1.1.29 | Apache-2.0 OR MIT |
| `linux-raw-sys` | 0.12.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `linux-raw-sys` | 0.4.15 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `litemap` | 0.8.2 | Unicode-3.0 |
| `lock_api` | 0.4.14 | Apache-2.0 OR MIT |
| `log` | 0.4.33 | Apache-2.0 OR MIT |
| `lru` | 0.12.5 | MIT |
| `lru` | 0.18.2 | MIT |
| `lzma-rs` | 0.3.0 | MIT |
| `lzma-rust2` | 0.15.8 | Apache-2.0 |
| `mac-notification-sys` | 0.6.15 | Apache-2.0 OR MIT |
| `mach2` | 0.4.3 | Apache-2.0 OR BSD-2-Clause OR MIT |
| `machine-uid` | 0.5.4 | MIT |
| `macro_rules_attribute` | 0.2.3 | Apache-2.0 OR MIT OR Zlib |
| `macro_rules_attribute-proc_macro` | 0.2.3 | Apache-2.0 OR MIT OR Zlib |
| `manyhow` | 0.11.4 | Apache-2.0 OR MIT |
| `manyhow-macros` | 0.11.4 | Apache-2.0 OR MIT |
| `markup5ever` | 0.38.0 | Apache-2.0 OR MIT |
| `matchers` | 0.2.0 | MIT |
| `matrixmultiply` | 0.3.11 | Apache-2.0 OR MIT |
| `maybe-owned` | 0.3.4 | Apache-2.0 OR MIT |
| `memchr` | 2.8.3 | MIT OR Unlicense |
| `memfd` | 0.6.5 | Apache-2.0 OR MIT |
| `memmap2` | 0.9.11 | Apache-2.0 OR MIT |
| `memoffset` | 0.9.1 | MIT |
| `mime` | 0.3.17 | Apache-2.0 OR MIT |
| `minimal-lexical` | 0.2.1 | Apache-2.0 OR MIT |
| `miniz_oxide` | 0.8.9 | Apache-2.0 OR MIT OR Zlib |
| `mio` | 0.8.11 | MIT |
| `mio` | 1.2.1 | MIT |
| `monostate` | 0.1.18 | Apache-2.0 OR MIT |
| `monostate-impl` | 0.1.18 | Apache-2.0 OR MIT |
| `muda` | 0.19.3 | Apache-2.0 OR MIT |
| `nalgebra` | 0.32.6 | BSD-3-Clause |
| `nalgebra-macros` | 0.2.2 | Apache-2.0 |
| `native-tls` | 0.2.18 | Apache-2.0 OR MIT |
| `ndarray` | 0.17.2 | Apache-2.0 OR MIT |
| `ndk` | 0.9.0 | Apache-2.0 OR MIT |
| `ndk-sys` | 0.6.0+11769913 | Apache-2.0 OR MIT |
| `netlink-packet-core` | 0.7.0 | MIT |
| `netlink-packet-sock-diag` | 0.4.2 | MIT |
| `netlink-packet-utils` | 0.5.2 | MIT |
| `netlink-sys` | 0.8.8 | MIT |
| `netstat2` | 0.11.2 | Apache-2.0 OR MIT |
| `new_debug_unreachable` | 1.0.6 | MIT |
| `nix` | 0.28.0 | MIT |
| `nix` | 0.29.0 | MIT |
| `nom` | 7.1.3 | MIT |
| `notify` | 6.1.1 | CC0-1.0 |
| `notify-rust` | 4.18.0 | Apache-2.0 OR MIT |
| `nu-ansi-term` | 0.50.3 | MIT |
| `num-bigint` | 0.4.8 | Apache-2.0 OR MIT |
| `num-complex` | 0.2.4 | Apache-2.0 OR MIT |
| `num-complex` | 0.4.6 | Apache-2.0 OR MIT |
| `num-conv` | 0.2.2 | Apache-2.0 OR MIT |
| `num-derive` | 0.3.3 | Apache-2.0 OR MIT |
| `num-integer` | 0.1.46 | Apache-2.0 OR MIT |
| `num-rational` | 0.4.2 | Apache-2.0 OR MIT |
| `num-traits` | 0.2.19 | Apache-2.0 OR MIT |
| `num_cpus` | 1.17.0 | Apache-2.0 OR MIT |
| `num_enum` | 0.7.6 | Apache-2.0 OR BSD-3-Clause OR MIT |
| `num_enum_derive` | 0.7.6 | Apache-2.0 OR BSD-3-Clause OR MIT |
| `number_prefix` | 0.4.0 | MIT |
| `objc2` | 0.6.4 | MIT |
| `objc2-app-kit` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-cloud-kit` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-core-data` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-core-foundation` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-core-graphics` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-core-image` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-core-location` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-core-text` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-encode` | 4.1.0 | MIT |
| `objc2-exception-helper` | 0.1.1 | Apache-2.0 OR MIT OR Zlib |
| `objc2-foundation` | 0.3.2 | MIT |
| `objc2-quartz-core` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-ui-kit` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-user-notifications` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `objc2-web-kit` | 0.3.2 | Apache-2.0 OR MIT OR Zlib |
| `object` | 0.38.1 | Apache-2.0 OR MIT |
| `once_cell` | 1.21.4 | Apache-2.0 OR MIT |
| `onig` | 6.5.3 | MIT |
| `onig_sys` | 69.9.3 | MIT |
| `opaque-debug` | 0.3.1 | Apache-2.0 OR MIT |
| `open` | 5.4.0 | MIT |
| `openssl` | 0.10.81 | Apache-2.0 |
| `openssl-macros` | 0.1.1 | Apache-2.0 OR MIT |
| `openssl-probe` | 0.2.1 | Apache-2.0 OR MIT |
| `openssl-sys` | 0.9.117 | MIT |
| `option-ext` | 0.2.0 | MPL-2.0 |
| `ordered-stream` | 0.2.0 | Apache-2.0 OR MIT |
| `ort` | 2.0.0-rc.13 | Apache-2.0 OR MIT |
| `ort-sys` | 2.0.0-rc.13 | Apache-2.0 OR MIT |
| `pango` | 0.18.3 | MIT |
| `pango-sys` | 0.18.0 | MIT |
| `parking` | 2.2.1 | Apache-2.0 OR MIT |
| `parking_lot` | 0.12.5 | Apache-2.0 OR MIT |
| `parking_lot_core` | 0.9.12 | Apache-2.0 OR MIT |
| `paste` | 1.0.15 | Apache-2.0 OR MIT |
| `pastey` | 0.2.3 | Apache-2.0 OR MIT |
| `pem-rfc7468` | 1.0.0 | Apache-2.0 OR MIT |
| `percent-encoding` | 2.3.2 | Apache-2.0 OR MIT |
| `petgraph` | 0.6.5 | Apache-2.0 OR MIT |
| `phf` | 0.13.1 | MIT |
| `phf_codegen` | 0.13.1 | MIT |
| `phf_generator` | 0.13.1 | MIT |
| `phf_macros` | 0.13.1 | MIT |
| `phf_shared` | 0.13.1 | MIT |
| `pin-project-lite` | 0.2.17 | Apache-2.0 OR MIT |
| `piper` | 0.2.5 | Apache-2.0 OR MIT |
| `pkg-config` | 0.3.33 | Apache-2.0 OR MIT |
| `plist` | 1.10.0 | MIT |
| `png` | 0.17.16 | Apache-2.0 OR MIT |
| `png` | 0.18.1 | Apache-2.0 OR MIT |
| `polling` | 3.11.0 | Apache-2.0 OR MIT |
| `polyval` | 0.6.2 | Apache-2.0 OR MIT |
| `portable-atomic` | 1.14.0 | Apache-2.0 OR MIT |
| `portable-atomic-util` | 0.2.7 | Apache-2.0 OR MIT |
| `portable-pty` | 0.9.0 | MIT |
| `postcard` | 1.1.3 | Apache-2.0 OR MIT |
| `potential_utf` | 0.1.5 | Unicode-3.0 |
| `powerfmt` | 0.2.0 | Apache-2.0 OR MIT |
| `ppv-lite86` | 0.2.21 | Apache-2.0 OR MIT |
| `precomputed-hash` | 0.1.1 | MIT |
| `prettyplease` | 0.2.37 | Apache-2.0 OR MIT |
| `proc-macro-crate` | 1.3.1 | Apache-2.0 OR MIT |
| `proc-macro-crate` | 2.0.2 | Apache-2.0 OR MIT |
| `proc-macro-crate` | 3.5.0 | Apache-2.0 OR MIT |
| `proc-macro-error` | 1.0.4 | Apache-2.0 OR MIT |
| `proc-macro-error-attr` | 1.0.4 | Apache-2.0 OR MIT |
| `proc-macro-utils` | 0.10.0 | Apache-2.0 OR MIT |
| `proc-macro2` | 1.0.106 | Apache-2.0 OR MIT |
| `prost` | 0.14.4 | Apache-2.0 |
| `prost-derive` | 0.14.4 | Apache-2.0 |
| `pulley-interpreter` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `pulley-macros` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `quick-xml` | 0.41.0 | MIT |
| `quote` | 1.0.46 | Apache-2.0 OR MIT |
| `r-efi` | 5.3.0 | Apache-2.0 OR LGPL-2.1-or-later OR MIT |
| `r-efi` | 6.0.0 | Apache-2.0 OR LGPL-2.1-or-later OR MIT |
| `rand` | 0.8.7 | Apache-2.0 OR MIT |
| `rand` | 0.9.5 | Apache-2.0 OR MIT |
| `rand_chacha` | 0.3.1 | Apache-2.0 OR MIT |
| `rand_chacha` | 0.9.0 | Apache-2.0 OR MIT |
| `rand_core` | 0.6.4 | Apache-2.0 OR MIT |
| `rand_core` | 0.9.5 | Apache-2.0 OR MIT |
| `rand_xoshiro` | 0.6.0 | Apache-2.0 OR MIT |
| `raw-window-handle` | 0.6.2 | Apache-2.0 OR MIT OR Zlib |
| `rawpointer` | 0.2.1 | Apache-2.0 OR MIT |
| `rayon` | 1.12.0 | Apache-2.0 OR MIT |
| `rayon-cond` | 0.4.0 | Apache-2.0 OR MIT |
| `rayon-core` | 1.13.0 | Apache-2.0 OR MIT |
| `redox_syscall` | 0.5.18 | MIT |
| `redox_users` | 0.4.6 | MIT |
| `redox_users` | 0.5.2 | MIT |
| `regalloc2` | 0.15.1 | Apache-2.0 WITH LLVM-exception |
| `regex` | 1.13.0 | Apache-2.0 OR MIT |
| `regex-automata` | 0.4.15 | Apache-2.0 OR MIT |
| `regex-syntax` | 0.8.11 | Apache-2.0 OR MIT |
| `reqwest` | 0.13.4 | Apache-2.0 OR MIT |
| `rfd` | 0.16.0 | MIT |
| `ring` | 0.17.14 | Apache-2.0 OR ISC |
| `rmp` | 0.8.15 | MIT |
| `rmp-serde` | 1.3.1 | MIT |
| `rmpv` | 1.3.1 | MIT |
| `rusqlite` | 0.32.1 | MIT |
| `rustc-demangle` | 0.1.28 | Apache-2.0 OR MIT |
| `rustc-hash` | 2.1.3 | Apache-2.0 OR MIT |
| `rustc_version` | 0.4.1 | Apache-2.0 OR MIT |
| `rustix` | 0.38.44 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `rustix` | 1.1.4 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `rustix-linux-procfs` | 0.1.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `rustls` | 0.23.41 | Apache-2.0 OR ISC OR MIT |
| `rustls-pki-types` | 1.15.0 | Apache-2.0 OR MIT |
| `rustls-webpki` | 0.103.13 | ISC |
| `rustversion` | 1.0.23 | Apache-2.0 OR MIT |
| `ryu` | 1.0.23 | Apache-2.0 OR BSL-1.0 |
| `safe_arch` | 0.7.4 | Apache-2.0 OR MIT OR Zlib |
| `safetensors` | 0.8.0 | Apache-2.0 |
| `same-file` | 1.0.6 | MIT OR Unlicense |
| `schannel` | 0.1.29 | MIT |
| `schemars` | 0.8.22 | MIT |
| `schemars_derive` | 0.8.22 | MIT |
| `scopeguard` | 1.2.0 | Apache-2.0 OR MIT |
| `security-framework` | 3.7.0 | Apache-2.0 OR MIT |
| `security-framework-sys` | 2.17.0 | Apache-2.0 OR MIT |
| `selectors` | 0.36.1 | MPL-2.0 |
| `semver` | 1.0.28 | Apache-2.0 OR MIT |
| `serde` | 1.0.228 | Apache-2.0 OR MIT |
| `serde-untagged` | 0.1.9 | Apache-2.0 OR MIT |
| `serde_core` | 1.0.228 | Apache-2.0 OR MIT |
| `serde_derive` | 1.0.228 | Apache-2.0 OR MIT |
| `serde_derive_internals` | 0.29.1 | Apache-2.0 OR MIT |
| `serde_json` | 1.0.150 | Apache-2.0 OR MIT |
| `serde_repr` | 0.1.20 | Apache-2.0 OR MIT |
| `serde_spanned` | 0.6.9 | Apache-2.0 OR MIT |
| `serde_spanned` | 1.1.1 | Apache-2.0 OR MIT |
| `serde_with` | 3.21.0 | Apache-2.0 OR MIT |
| `serde_with_macros` | 3.21.0 | Apache-2.0 OR MIT |
| `serde_yaml` | 0.9.34+deprecated | Apache-2.0 OR MIT |
| `serial2` | 0.2.37 | Apache-2.0 OR BSD-2-Clause |
| `serialize-to-javascript` | 0.1.2 | Apache-2.0 OR MIT |
| `serialize-to-javascript-impl` | 0.1.2 | Apache-2.0 OR MIT |
| `servo_arc` | 0.4.3 | Apache-2.0 OR MIT |
| `sha2` | 0.10.9 | Apache-2.0 OR MIT |
| `sharded-slab` | 0.1.7 | MIT |
| `shared_library` | 0.1.9 | Apache-2.0 OR MIT |
| `shell-words` | 1.1.1 | Apache-2.0 OR MIT |
| `shlex` | 1.3.0 | Apache-2.0 OR MIT |
| `shlex` | 2.0.1 | Apache-2.0 OR MIT |
| `signal-hook-registry` | 1.4.8 | Apache-2.0 OR MIT |
| `signature` | 2.2.0 | Apache-2.0 OR MIT |
| `simba` | 0.8.1 | Apache-2.0 |
| `simd-adler32` | 0.3.9 | MIT |
| `siphasher` | 1.0.3 | Apache-2.0 OR MIT |
| `sized-chunks` | 0.6.5 | MPL-2.0+ |
| `slab` | 0.4.12 | MIT |
| `smallvec` | 1.15.2 | Apache-2.0 OR MIT |
| `socket2` | 0.6.4 | Apache-2.0 OR MIT |
| `socks` | 0.3.4 | Apache-2.0 OR MIT |
| `softbuffer` | 0.4.8 | Apache-2.0 OR MIT |
| `soup3` | 0.5.0 | MIT |
| `soup3-sys` | 0.5.0 | MIT |
| `spm_precompiled` | 0.1.4 | Apache-2.0 |
| `sprs` | 0.11.5 | Apache-2.0 OR MIT |
| `stable_deref_trait` | 1.2.1 | Apache-2.0 OR MIT |
| `static_assertions` | 1.1.0 | Apache-2.0 OR MIT |
| `string_cache` | 0.9.0 | Apache-2.0 OR MIT |
| `string_cache_codegen` | 0.6.1 | Apache-2.0 OR MIT |
| `strsim` | 0.11.1 | MIT |
| `subtle` | 2.6.1 | BSD-3-Clause |
| `swift-rs` | 1.0.7 | Apache-2.0 OR MIT |
| `symlink` | 0.1.0 | Apache-2.0 OR MIT |
| `syn` | 1.0.109 | Apache-2.0 OR MIT |
| `syn` | 2.0.118 | Apache-2.0 OR MIT |
| `syn` | 3.0.3 | Apache-2.0 OR MIT |
| `sync_wrapper` | 1.0.2 | Apache-2.0 |
| `synstructure` | 0.13.2 | MIT |
| `system-deps` | 6.2.2 | Apache-2.0 OR MIT |
| `system-interface` | 0.27.3 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `tao` | 0.35.3 | Apache-2.0 |
| `tao-macros` | 0.1.3 | Apache-2.0 OR MIT |
| `tar` | 0.4.46 | Apache-2.0 OR MIT |
| `target-lexicon` | 0.12.16 | Apache-2.0 WITH LLVM-exception |
| `target-lexicon` | 0.13.5 | Apache-2.0 WITH LLVM-exception |
| `tauri` | 2.11.5 | Apache-2.0 OR MIT |
| `tauri-build` | 2.6.3 | Apache-2.0 OR MIT |
| `tauri-codegen` | 2.6.3 | Apache-2.0 OR MIT |
| `tauri-macros` | 2.6.3 | Apache-2.0 OR MIT |
| `tauri-plugin` | 2.6.3 | Apache-2.0 OR MIT |
| `tauri-plugin-dialog` | 2.7.1 | Apache-2.0 OR MIT |
| `tauri-plugin-fs` | 2.5.1 | Apache-2.0 OR MIT |
| `tauri-plugin-notification` | 2.3.3 | Apache-2.0 OR MIT |
| `tauri-plugin-opener` | 2.5.4 | Apache-2.0 OR MIT |
| `tauri-plugin-single-instance` | 2.4.3 | Apache-2.0 OR MIT |
| `tauri-runtime` | 2.11.3 | Apache-2.0 OR MIT |
| `tauri-runtime-wry` | 2.11.4 | Apache-2.0 OR MIT |
| `tauri-utils` | 2.9.3 | Apache-2.0 OR MIT |
| `tauri-winres` | 0.3.6 | MIT |
| `tauri-winrt-notification` | 0.7.3 | Apache-2.0 OR MIT |
| `tempfile` | 3.27.0 | Apache-2.0 OR MIT |
| `tendril` | 0.5.1 | Apache-2.0 OR MIT |
| `termcolor` | 1.4.1 | MIT OR Unlicense |
| `terminal_size` | 0.4.4 | Apache-2.0 OR MIT |
| `thiserror` | 1.0.69 | Apache-2.0 OR MIT |
| `thiserror` | 2.0.18 | Apache-2.0 OR MIT |
| `thiserror-impl` | 1.0.69 | Apache-2.0 OR MIT |
| `thiserror-impl` | 2.0.18 | Apache-2.0 OR MIT |
| `thread_local` | 1.1.10 | Apache-2.0 OR MIT |
| `time` | 0.3.53 | Apache-2.0 OR MIT |
| `time-core` | 0.1.9 | Apache-2.0 OR MIT |
| `time-macros` | 0.2.31 | Apache-2.0 OR MIT |
| `tinystr` | 0.8.3 | Unicode-3.0 |
| `tokenizers` | 0.22.2 | Apache-2.0 |
| `tokio` | 1.52.3 | MIT |
| `tokio-macros` | 2.7.2 | MIT |
| `tokio-util` | 0.7.18 | MIT |
| `toml` | 0.8.2 | Apache-2.0 OR MIT |
| `toml` | 0.9.12+spec-1.1.0 | Apache-2.0 OR MIT |
| `toml` | 1.1.2+spec-1.1.0 | Apache-2.0 OR MIT |
| `toml_datetime` | 0.6.3 | Apache-2.0 OR MIT |
| `toml_datetime` | 0.7.5+spec-1.1.0 | Apache-2.0 OR MIT |
| `toml_datetime` | 1.1.1+spec-1.1.0 | Apache-2.0 OR MIT |
| `toml_edit` | 0.19.15 | Apache-2.0 OR MIT |
| `toml_edit` | 0.20.2 | Apache-2.0 OR MIT |
| `toml_edit` | 0.25.12+spec-1.1.0 | Apache-2.0 OR MIT |
| `toml_parser` | 1.1.2+spec-1.1.0 | Apache-2.0 OR MIT |
| `toml_writer` | 1.1.1+spec-1.1.0 | Apache-2.0 OR MIT |
| `tower` | 0.5.3 | MIT |
| `tower-http` | 0.6.11 | MIT |
| `tower-layer` | 0.3.3 | MIT |
| `tower-service` | 0.3.3 | MIT |
| `tracing` | 0.1.44 | MIT |
| `tracing-appender` | 0.2.5 | MIT |
| `tracing-attributes` | 0.1.31 | MIT |
| `tracing-core` | 0.1.36 | MIT |
| `tracing-log` | 0.2.0 | MIT |
| `tracing-subscriber` | 0.3.23 | MIT |
| `trash` | 5.2.7 | MIT |
| `try-lock` | 0.2.5 | MIT |
| `typeid` | 1.0.3 | Apache-2.0 OR MIT |
| `typenum` | 1.20.1 | Apache-2.0 OR MIT |
| `uds_windows` | 1.2.1 | MIT |
| `unic-char-property` | 0.9.0 | Apache-2.0 OR MIT |
| `unic-char-range` | 0.9.0 | Apache-2.0 OR MIT |
| `unic-common` | 0.9.0 | Apache-2.0 OR MIT |
| `unic-ucd-ident` | 0.9.0 | Apache-2.0 OR MIT |
| `unic-ucd-version` | 0.9.0 | Apache-2.0 OR MIT |
| `unicode-ident` | 1.0.24 | Apache-2.0 OR MIT OR Unicode-3.0 |
| `unicode-normalization-alignments` | 0.1.12 | Apache-2.0 OR MIT |
| `unicode-segmentation` | 1.13.3 | Apache-2.0 OR MIT |
| `unicode-width` | 0.2.2 | Apache-2.0 OR MIT |
| `unicode-xid` | 0.2.6 | Apache-2.0 OR MIT |
| `unicode_categories` | 0.1.1 | Apache-2.0 OR MIT |
| `universal-hash` | 0.5.1 | Apache-2.0 OR MIT |
| `unsafe-libyaml` | 0.2.11 | MIT |
| `untrusted` | 0.9.0 | ISC |
| `ureq` | 2.12.1 | Apache-2.0 OR MIT |
| `ureq` | 3.3.0 | Apache-2.0 OR MIT |
| `ureq-proto` | 0.6.0 | Apache-2.0 OR MIT |
| `url` | 2.5.8 | Apache-2.0 OR MIT |
| `urlencoding` | 2.1.3 | MIT |
| `urlpattern` | 0.3.0 | MIT |
| `utf8-zero` | 0.8.1 | Apache-2.0 OR MIT |
| `utf8_iter` | 1.0.4 | Apache-2.0 OR MIT |
| `uuid` | 1.23.5 | Apache-2.0 OR MIT |
| `vcpkg` | 0.2.15 | Apache-2.0 OR MIT |
| `version-compare` | 0.2.1 | MIT |
| `version_check` | 0.9.5 | Apache-2.0 OR MIT |
| `vswhom` | 0.1.0 | MIT |
| `vswhom-sys` | 0.1.3 | MIT |
| `walkdir` | 2.5.0 | MIT OR Unlicense |
| `want` | 0.3.1 | MIT |
| `wasi` | 0.11.1+wasi-snapshot-preview1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wasi-common` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasip2` | 1.0.4+wasi-0.2.12 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wasm-bindgen` | 0.2.126 | Apache-2.0 OR MIT |
| `wasm-bindgen-futures` | 0.4.76 | Apache-2.0 OR MIT |
| `wasm-bindgen-macro` | 0.2.126 | Apache-2.0 OR MIT |
| `wasm-bindgen-macro-support` | 0.2.126 | Apache-2.0 OR MIT |
| `wasm-bindgen-shared` | 0.2.126 | Apache-2.0 OR MIT |
| `wasm-compose` | 0.245.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wasm-encoder` | 0.245.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wasm-encoder` | 0.253.0 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wasm-streams` | 0.5.0 | Apache-2.0 OR MIT |
| `wasmparser` | 0.245.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wasmprinter` | 0.245.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wasmtime` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-environ` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-cache` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-component-macro` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-component-util` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-core` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-cranelift` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-fiber` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-jit-debug` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-jit-icache-coherence` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-unwinder` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-versioned-export-macros` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wasmtime-internal-wit-bindgen` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wast` | 253.0.0 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wast` | 35.0.2 | Apache-2.0 WITH LLVM-exception |
| `wat` | 1.253.0 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `web-sys` | 0.3.103 | Apache-2.0 OR MIT |
| `web-time` | 1.1.0 | Apache-2.0 OR MIT |
| `web_atoms` | 0.2.5 | Apache-2.0 OR MIT |
| `webkit2gtk` | 2.0.2 | MIT |
| `webkit2gtk-sys` | 2.0.2 | MIT |
| `webpki-root-certs` | 1.0.9 | CDLA-Permissive-2.0 |
| `webpki-roots` | 0.26.11 | CDLA-Permissive-2.0 |
| `webpki-roots` | 1.0.8 | CDLA-Permissive-2.0 |
| `webview2-com` | 0.38.2 | MIT |
| `webview2-com-macros` | 0.8.1 | MIT |
| `webview2-com-sys` | 0.38.2 | MIT |
| `wide` | 0.7.33 | Apache-2.0 OR MIT OR Zlib |
| `wiggle` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wiggle-generate` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `wiggle-macro` | 43.0.2 | Apache-2.0 WITH LLVM-exception |
| `winapi` | 0.3.9 | Apache-2.0 OR MIT |
| `winapi-i686-pc-windows-gnu` | 0.4.0 | Apache-2.0 OR MIT |
| `winapi-util` | 0.1.11 | MIT OR Unlicense |
| `winapi-x86_64-pc-windows-gnu` | 0.4.0 | Apache-2.0 OR MIT |
| `window-vibrancy` | 0.6.0 | Apache-2.0 OR MIT |
| `windows` | 0.56.0 | Apache-2.0 OR MIT |
| `windows` | 0.61.3 | Apache-2.0 OR MIT |
| `windows-collections` | 0.2.0 | Apache-2.0 OR MIT |
| `windows-core` | 0.56.0 | Apache-2.0 OR MIT |
| `windows-core` | 0.61.2 | Apache-2.0 OR MIT |
| `windows-core` | 0.62.2 | Apache-2.0 OR MIT |
| `windows-future` | 0.2.1 | Apache-2.0 OR MIT |
| `windows-implement` | 0.56.0 | Apache-2.0 OR MIT |
| `windows-implement` | 0.60.2 | Apache-2.0 OR MIT |
| `windows-interface` | 0.56.0 | Apache-2.0 OR MIT |
| `windows-interface` | 0.59.3 | Apache-2.0 OR MIT |
| `windows-link` | 0.1.3 | Apache-2.0 OR MIT |
| `windows-link` | 0.2.1 | Apache-2.0 OR MIT |
| `windows-numerics` | 0.2.0 | Apache-2.0 OR MIT |
| `windows-registry` | 0.6.1 | Apache-2.0 OR MIT |
| `windows-result` | 0.1.2 | Apache-2.0 OR MIT |
| `windows-result` | 0.3.4 | Apache-2.0 OR MIT |
| `windows-result` | 0.4.1 | Apache-2.0 OR MIT |
| `windows-strings` | 0.4.2 | Apache-2.0 OR MIT |
| `windows-strings` | 0.5.1 | Apache-2.0 OR MIT |
| `windows-sys` | 0.45.0 | Apache-2.0 OR MIT |
| `windows-sys` | 0.48.0 | Apache-2.0 OR MIT |
| `windows-sys` | 0.52.0 | Apache-2.0 OR MIT |
| `windows-sys` | 0.59.0 | Apache-2.0 OR MIT |
| `windows-sys` | 0.60.2 | Apache-2.0 OR MIT |
| `windows-sys` | 0.61.2 | Apache-2.0 OR MIT |
| `windows-targets` | 0.42.2 | Apache-2.0 OR MIT |
| `windows-targets` | 0.48.5 | Apache-2.0 OR MIT |
| `windows-targets` | 0.52.6 | Apache-2.0 OR MIT |
| `windows-targets` | 0.53.5 | Apache-2.0 OR MIT |
| `windows-threading` | 0.1.0 | Apache-2.0 OR MIT |
| `windows-version` | 0.1.7 | Apache-2.0 OR MIT |
| `windows_aarch64_gnullvm` | 0.42.2 | Apache-2.0 OR MIT |
| `windows_aarch64_gnullvm` | 0.48.5 | Apache-2.0 OR MIT |
| `windows_aarch64_gnullvm` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_aarch64_gnullvm` | 0.53.1 | Apache-2.0 OR MIT |
| `windows_aarch64_msvc` | 0.42.2 | Apache-2.0 OR MIT |
| `windows_aarch64_msvc` | 0.48.5 | Apache-2.0 OR MIT |
| `windows_aarch64_msvc` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_aarch64_msvc` | 0.53.1 | Apache-2.0 OR MIT |
| `windows_i686_gnu` | 0.42.2 | Apache-2.0 OR MIT |
| `windows_i686_gnu` | 0.48.5 | Apache-2.0 OR MIT |
| `windows_i686_gnu` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_i686_gnu` | 0.53.1 | Apache-2.0 OR MIT |
| `windows_i686_gnullvm` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_i686_gnullvm` | 0.53.1 | Apache-2.0 OR MIT |
| `windows_i686_msvc` | 0.42.2 | Apache-2.0 OR MIT |
| `windows_i686_msvc` | 0.48.5 | Apache-2.0 OR MIT |
| `windows_i686_msvc` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_i686_msvc` | 0.53.1 | Apache-2.0 OR MIT |
| `windows_x86_64_gnu` | 0.42.2 | Apache-2.0 OR MIT |
| `windows_x86_64_gnu` | 0.48.5 | Apache-2.0 OR MIT |
| `windows_x86_64_gnu` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_x86_64_gnu` | 0.53.1 | Apache-2.0 OR MIT |
| `windows_x86_64_gnullvm` | 0.42.2 | Apache-2.0 OR MIT |
| `windows_x86_64_gnullvm` | 0.48.5 | Apache-2.0 OR MIT |
| `windows_x86_64_gnullvm` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_x86_64_gnullvm` | 0.53.1 | Apache-2.0 OR MIT |
| `windows_x86_64_msvc` | 0.42.2 | Apache-2.0 OR MIT |
| `windows_x86_64_msvc` | 0.48.5 | Apache-2.0 OR MIT |
| `windows_x86_64_msvc` | 0.52.6 | Apache-2.0 OR MIT |
| `windows_x86_64_msvc` | 0.53.1 | Apache-2.0 OR MIT |
| `winnow` | 0.5.40 | MIT |
| `winnow` | 0.7.15 | MIT |
| `winnow` | 1.0.3 | MIT |
| `winreg` | 0.10.1 | MIT |
| `winreg` | 0.55.0 | MIT |
| `winx` | 0.36.4 | Apache-2.0 WITH LLVM-exception |
| `wit-bindgen` | 0.57.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `wit-parser` | 0.245.1 | Apache-2.0 OR Apache-2.0 WITH LLVM-exception OR MIT |
| `witx` | 0.9.1 | Apache-2.0 |
| `writeable` | 0.6.3 | Unicode-3.0 |
| `wry` | 0.55.1 | Apache-2.0 OR MIT |
| `x11` | 2.21.0 | MIT |
| `x11-dl` | 2.21.0 | MIT |
| `xattr` | 1.6.1 | Apache-2.0 OR MIT |
| `yoke` | 0.8.3 | Unicode-3.0 |
| `yoke-derive` | 0.8.2 | Unicode-3.0 |
| `zbus` | 5.17.0 | MIT |
| `zbus_macros` | 5.17.0 | MIT |
| `zbus_names` | 4.3.3 | MIT |
| `zerocopy` | 0.8.54 | Apache-2.0 OR BSD-2-Clause OR MIT |
| `zerofrom` | 0.1.8 | Unicode-3.0 |
| `zerofrom-derive` | 0.1.7 | Unicode-3.0 |
| `zeroize` | 1.9.0 | Apache-2.0 OR MIT |
| `zerotrie` | 0.2.4 | Unicode-3.0 |
| `zerovec` | 0.11.6 | Unicode-3.0 |
| `zerovec-derive` | 0.11.3 | Unicode-3.0 |
| `zip` | 2.4.2 | MIT |
| `zmij` | 1.0.22 | MIT |
| `zopfli` | 0.8.3 | Apache-2.0 |
| `zstd` | 0.13.3 | MIT |
| `zstd-safe` | 7.2.4 | Apache-2.0 OR MIT |
| `zstd-sys` | 2.0.16+zstd.1.5.7 | Apache-2.0 OR MIT |
| `zvariant` | 5.13.0 | MIT |
| `zvariant_derive` | 5.13.0 | MIT |
| `zvariant_utils` | 3.5.0 | MIT |

## Frontend dependencies (560)

| Component | Version | License |
|---|---|---|
| `@antfu/install-pkg` | 1.1.0 | MIT |
| `@asamuzakjp/css-color` | 5.1.11 | MIT |
| `@asamuzakjp/dom-selector` | 7.1.1 | MIT |
| `@asamuzakjp/generational-cache` | 1.0.1 | MIT |
| `@asamuzakjp/nwsapi` | 2.3.9 | MIT |
| `@babel/generator` | 8.0.0 | MIT |
| `@babel/helper-string-parser` | 8.0.0 | MIT |
| `@babel/helper-validator-identifier` | 8.0.4 | MIT |
| `@babel/parser` | 7.29.7 | MIT |
| `@babel/types` | 8.0.4 | MIT |
| `@biomejs/biome` | 2.5.6 | MIT OR Apache-2.0 |
| `@biomejs/cli-darwin-arm64` | 2.5.6 | MIT OR Apache-2.0 |
| `@braintree/sanitize-url` | 7.1.2 | MIT |
| `@bramus/specificity` | 2.4.2 | MIT |
| `@chevrotain/types` | 11.1.2 | Apache-2.0 |
| `@codemirror/autocomplete` | 6.20.3 | MIT |
| `@codemirror/commands` | 6.10.4 | MIT |
| `@codemirror/lang-angular` | 0.1.4 | MIT |
| `@codemirror/lang-cpp` | 6.0.3 | MIT |
| `@codemirror/lang-css` | 6.3.1 | MIT |
| `@codemirror/lang-go` | 6.0.1 | MIT |
| `@codemirror/lang-html` | 6.4.11 | MIT |
| `@codemirror/lang-java` | 6.0.2 | MIT |
| `@codemirror/lang-javascript` | 6.2.5 | MIT |
| `@codemirror/lang-jinja` | 6.0.1 | MIT |
| `@codemirror/lang-json` | 6.0.2 | MIT |
| `@codemirror/lang-less` | 6.0.2 | MIT |
| `@codemirror/lang-liquid` | 6.3.2 | MIT |
| `@codemirror/lang-markdown` | 6.5.0 | MIT |
| `@codemirror/lang-php` | 6.0.2 | MIT |
| `@codemirror/lang-python` | 6.2.1 | MIT |
| `@codemirror/lang-rust` | 6.0.2 | MIT |
| `@codemirror/lang-sass` | 6.0.2 | MIT |
| `@codemirror/lang-sql` | 6.10.0 | MIT |
| `@codemirror/lang-vue` | 0.1.3 | MIT |
| `@codemirror/lang-wast` | 6.0.2 | MIT |
| `@codemirror/lang-xml` | 6.1.0 | MIT |
| `@codemirror/lang-yaml` | 6.1.3 | MIT |
| `@codemirror/language` | 6.12.4 | MIT |
| `@codemirror/language-data` | 6.5.2 | MIT |
| `@codemirror/legacy-modes` | 6.5.3 | MIT |
| `@codemirror/lint` | 6.9.7 | MIT |
| `@codemirror/lsp-client` | 6.2.5 | MIT |
| `@codemirror/merge` | 6.12.2 | MIT |
| `@codemirror/search` | 6.7.1 | MIT |
| `@codemirror/state` | 6.7.1 | MIT |
| `@codemirror/view` | 6.43.6 | MIT |
| `@csstools/color-helpers` | 6.1.0 | MIT-0 |
| `@csstools/css-calc` | 3.2.1 | MIT |
| `@csstools/css-color-parser` | 4.1.9 | MIT |
| `@csstools/css-parser-algorithms` | 4.0.0 | MIT |
| `@csstools/css-syntax-patches-for-csstree` | 1.1.6 | MIT-0 |
| `@csstools/css-tokenizer` | 4.0.0 | MIT |
| `@emmetio/abbreviation` | 2.3.3 | MIT |
| `@emmetio/codemirror6-plugin` | 0.4.1 | Unknown |
| `@emmetio/css-abbreviation` | 2.1.8 | MIT |
| `@emmetio/math-expression` | 1.0.5 | MIT |
| `@emmetio/scanner` | 1.0.4 | MIT |
| `@esbuild/darwin-arm64` | 0.28.1 | MIT |
| `@exodus/bytes` | 1.15.1 | MIT |
| `@fontsource/poppins` | 5.3.0 | OFL-1.1 |
| `@iconify/types` | 2.0.0 | MIT |
| `@iconify/utils` | 3.1.4 | MIT |
| `@inversifyjs/common` | 1.5.2 | MIT |
| `@inversifyjs/container` | 1.15.0 | MIT |
| `@inversifyjs/core` | 9.2.0 | MIT |
| `@inversifyjs/plugin` | 0.2.0 | MIT |
| `@inversifyjs/prototype-utils` | 0.1.3 | MIT |
| `@inversifyjs/reflect-metadata-utils` | 1.4.1 | MIT |
| `@isaacs/cliui` | 8.0.2 | ISC |
| `@javascript-obfuscator/escodegen` | 2.4.2 | BSD-2-Clause |
| `@javascript-obfuscator/estraverse` | 5.4.0 | BSD-2-Clause |
| `@jridgewell/gen-mapping` | 0.3.13 | MIT |
| `@jridgewell/remapping` | 2.3.5 | MIT |
| `@jridgewell/resolve-uri` | 3.1.2 | MIT |
| `@jridgewell/sourcemap-codec` | 1.5.5 | MIT |
| `@jridgewell/trace-mapping` | 0.3.31 | MIT |
| `@lezer/common` | 1.5.2 | MIT |
| `@lezer/cpp` | 1.1.6 | MIT |
| `@lezer/css` | 1.3.4 | MIT |
| `@lezer/go` | 1.0.1 | MIT |
| `@lezer/highlight` | 1.2.3 | MIT |
| `@lezer/html` | 1.3.13 | MIT |
| `@lezer/java` | 1.1.3 | MIT |
| `@lezer/javascript` | 1.5.4 | MIT |
| `@lezer/json` | 1.0.3 | MIT |
| `@lezer/lr` | 1.4.10 | MIT |
| `@lezer/markdown` | 1.6.4 | MIT |
| `@lezer/php` | 1.0.5 | MIT |
| `@lezer/python` | 1.1.19 | MIT |
| `@lezer/rust` | 1.0.2 | MIT |
| `@lezer/sass` | 1.1.0 | MIT |
| `@lezer/xml` | 1.0.6 | MIT |
| `@lezer/yaml` | 1.0.4 | MIT |
| `@lucide/vue` | 1.28.0 | ISC |
| `@marijn/find-cluster-break` | 1.0.3 | MIT |
| `@mdit/plugin-emoji` | 1.1.1 | MIT |
| `@mermaid-js/parser` | 1.2.0 | MIT |
| `@one-ini/wasm` | 0.1.1 | MIT |
| `@pixi/colord` | 2.9.6 | MIT |
| `@pkgjs/parseargs` | 0.11.0 | MIT |
| `@primeuix/styled` | 0.7.4 | MIT |
| `@primeuix/styles` | 2.0.3 | MIT |
| `@primeuix/themes` | 2.0.3 | MIT |
| `@primeuix/utils` | 0.6.4 | MIT |
| `@primevue/auto-import-resolver` | 4.5.5 | MIT |
| `@primevue/core` | 4.5.5 | MIT |
| `@primevue/icons` | 4.5.5 | MIT |
| `@primevue/metadata` | 4.5.5 | MIT |
| `@replit/codemirror-indentation-markers` | 6.5.3 | MIT |
| `@replit/codemirror-vim` | 6.4.0 | MIT |
| `@replit/codemirror-vim-core` | 0.1.0 | MIT |
| `@rolldown/pluginutils` | 1.0.1 | MIT |
| `@rollup/rollup-darwin-arm64` | 4.62.2 | MIT |
| `@tanstack/match-sorter-utils` | 8.19.4 | MIT |
| `@tanstack/query-core` | 5.101.4 | MIT |
| `@tanstack/vue-query` | 5.101.4 | MIT |
| `@tauri-apps/api` | 2.11.1 | Apache-2.0 OR MIT |
| `@tauri-apps/cli` | 2.11.4 | Apache-2.0 OR MIT |
| `@tauri-apps/cli-darwin-arm64` | 2.11.4 | Apache-2.0 OR MIT |
| `@tauri-apps/plugin-dialog` | 2.7.2 | MIT OR Apache-2.0 |
| `@tauri-apps/plugin-notification` | 2.3.3 | MIT OR Apache-2.0 |
| `@tauri-apps/plugin-opener` | 2.5.4 | MIT OR Apache-2.0 |
| `@types/chai` | 5.2.3 | MIT |
| `@types/d3` | 7.4.3 | MIT |
| `@types/d3-array` | 3.2.2 | MIT |
| `@types/d3-axis` | 3.0.6 | MIT |
| `@types/d3-brush` | 3.0.6 | MIT |
| `@types/d3-chord` | 3.0.6 | MIT |
| `@types/d3-color` | 3.1.3 | MIT |
| `@types/d3-contour` | 3.0.6 | MIT |
| `@types/d3-delaunay` | 6.0.4 | MIT |
| `@types/d3-dispatch` | 3.0.7 | MIT |
| `@types/d3-drag` | 3.0.7 | MIT |
| `@types/d3-dsv` | 3.0.7 | MIT |
| `@types/d3-ease` | 3.0.2 | MIT |
| `@types/d3-fetch` | 3.0.7 | MIT |
| `@types/d3-force` | 3.0.10 | MIT |
| `@types/d3-format` | 3.0.4 | MIT |
| `@types/d3-geo` | 3.1.0 | MIT |
| `@types/d3-hierarchy` | 3.1.7 | MIT |
| `@types/d3-interpolate` | 3.0.4 | MIT |
| `@types/d3-path` | 3.1.1 | MIT |
| `@types/d3-polygon` | 3.0.2 | MIT |
| `@types/d3-quadtree` | 3.0.6 | MIT |
| `@types/d3-random` | 3.0.4 | MIT |
| `@types/d3-scale` | 4.0.9 | MIT |
| `@types/d3-scale-chromatic` | 3.1.0 | MIT |
| `@types/d3-selection` | 3.0.11 | MIT |
| `@types/d3-shape` | 3.1.8 | MIT |
| `@types/d3-time` | 3.0.4 | MIT |
| `@types/d3-time-format` | 4.0.3 | MIT |
| `@types/d3-timer` | 3.0.2 | MIT |
| `@types/d3-transition` | 3.0.9 | MIT |
| `@types/d3-zoom` | 3.0.8 | MIT |
| `@types/deep-eql` | 4.0.2 | MIT |
| `@types/earcut` | 3.0.0 | MIT |
| `@types/estree` | 1.0.9 | MIT |
| `@types/geojson` | 7946.0.16 | MIT |
| `@types/jsesc` | 2.5.1 | MIT |
| `@types/linkify-it` | 5.0.0 | MIT |
| `@types/markdown-it` | 14.1.2 | MIT |
| `@types/markdown-it-container` | 4.0.0 | MIT |
| `@types/markdown-it-footnote` | 3.0.4 | MIT |
| `@types/mdurl` | 2.0.0 | MIT |
| `@types/minimatch` | 3.0.5 | MIT |
| `@types/node` | 26.1.2 | MIT |
| `@types/trusted-types` | 2.0.7 | MIT |
| `@types/validator` | 13.15.10 | MIT |
| `@types/web-bluetooth` | 0.0.20 | MIT |
| `@upsetjs/venn.js` | 2.0.0 | MIT |
| `@vercel/blob` | 2.8.0 | Apache-2.0 |
| `@vercel/cli-config` | 0.2.4 | Apache-2.0 |
| `@vercel/cli-exec` | 1.0.1 | Apache-2.0 |
| `@vercel/oidc` | 3.8.5 | Apache-2.0 |
| `@vitejs/plugin-vue` | 6.0.8 | MIT |
| `@vitest/expect` | 3.2.6 | MIT |
| `@vitest/mocker` | 3.2.6 | MIT |
| `@vitest/pretty-format` | 3.2.6 | MIT |
| `@vitest/runner` | 3.2.6 | MIT |
| `@vitest/snapshot` | 3.2.6 | MIT |
| `@vitest/spy` | 3.2.6 | MIT |
| `@vitest/utils` | 3.2.6 | MIT |
| `@volar/language-core` | 2.4.15 | MIT |
| `@volar/source-map` | 2.4.15 | MIT |
| `@volar/typescript` | 2.4.15 | MIT |
| `@vscode/markdown-it-katex` | 1.1.2 | MIT |
| `@vue-flow/background` | 1.3.2 | MIT |
| `@vue-flow/core` | 1.48.2 | MIT |
| `@vue-macros/common` | 3.1.4 | MIT |
| `@vue/compiler-core` | 3.5.39 | MIT |
| `@vue/compiler-dom` | 3.5.39 | MIT |
| `@vue/compiler-sfc` | 3.5.40 | MIT |
| `@vue/compiler-ssr` | 3.5.39 | MIT |
| `@vue/compiler-vue2` | 2.7.16 | MIT |
| `@vue/devtools-api` | 6.6.4 | MIT |
| `@vue/devtools-kit` | 7.7.10 | MIT |
| `@vue/devtools-shared` | 7.7.10 | MIT |
| `@vue/language-core` | 2.2.12 | MIT |
| `@vue/reactivity` | 3.5.40 | MIT |
| `@vue/runtime-core` | 3.5.40 | MIT |
| `@vue/runtime-dom` | 3.5.40 | MIT |
| `@vue/server-renderer` | 3.5.39 | MIT |
| `@vue/shared` | 3.5.40 | MIT |
| `@vue/test-utils` | 2.4.11 | MIT |
| `@vueuse/core` | 10.11.1 | MIT |
| `@vueuse/metadata` | 10.11.1 | MIT |
| `@vueuse/shared` | 10.11.1 | MIT |
| `@webgpu/types` | 0.1.71 | BSD-3-Clause |
| `@xmldom/xmldom` | 0.8.13 | MIT |
| `@xterm/addon-fit` | 0.11.0 | MIT |
| `@xterm/addon-webgl` | 0.19.0 | MIT |
| `@xterm/xterm` | 6.0.0 | MIT |
| `abbrev` | 2.0.0 | ISC |
| `acorn` | 8.17.0 | MIT |
| `acorn-import-attributes` | 1.9.5 | MIT |
| `alien-signals` | 1.0.13 | MIT |
| `ansi-regex` | 6.2.2 | MIT |
| `ansi-styles` | 6.2.3 | MIT |
| `argparse` | 2.0.1 | Python-2.0 |
| `array-differ` | 3.0.0 | MIT |
| `array-union` | 2.1.0 | MIT |
| `arrify` | 2.0.1 | MIT |
| `assert` | 2.1.0 | MIT |
| `assertion-error` | 2.0.1 | MIT |
| `ast-kit` | 2.2.0 | MIT |
| `ast-walker-scope` | 0.9.0 | MIT |
| `async-retry` | 1.3.3 | MIT |
| `available-typed-arrays` | 1.0.7 | MIT |
| `balanced-match` | 1.0.2 | MIT |
| `bidi-js` | 1.0.3 | MIT |
| `birpc` | 2.9.0 | MIT |
| `brace-expansion` | 2.1.4 | MIT |
| `cac` | 6.7.14 | MIT |
| `call-bind` | 1.0.9 | MIT |
| `call-bind-apply-helpers` | 1.0.2 | MIT |
| `call-bound` | 1.0.4 | MIT |
| `chai` | 5.3.3 | MIT |
| `chalk` | 4.1.2 | MIT |
| `chance` | 1.1.13 | MIT |
| `char-regex` | 1.0.2 | MIT |
| `charenc` | 0.0.2 | BSD-3-Clause |
| `check-error` | 2.1.3 | MIT |
| `chokidar` | 5.0.0 | MIT |
| `class-validator` | 0.14.3 | MIT |
| `codemirror` | 6.0.2 | MIT |
| `color-convert` | 2.0.1 | MIT |
| `color-name` | 1.1.4 | MIT |
| `commander` | 8.3.0 | MIT |
| `confbox` | 0.2.4 | MIT |
| `config-chain` | 1.1.13 | MIT |
| `copy-anything` | 4.0.5 | MIT |
| `cose-base` | 1.0.3 | MIT |
| `crelt` | 1.0.7 | MIT |
| `cross-spawn` | 7.0.6 | MIT |
| `crypt` | 0.0.2 | BSD-3-Clause |
| `css-tree` | 3.2.1 | MIT |
| `csstype` | 3.2.3 | MIT |
| `cytoscape` | 3.34.0 | MIT |
| `cytoscape-cose-bilkent` | 4.1.0 | MIT |
| `cytoscape-fcose` | 2.2.0 | MIT |
| `d3` | 7.9.0 | ISC |
| `d3-array` | 3.2.4 | ISC |
| `d3-axis` | 3.0.0 | ISC |
| `d3-brush` | 3.0.0 | ISC |
| `d3-chord` | 3.0.1 | ISC |
| `d3-color` | 3.1.0 | ISC |
| `d3-contour` | 4.0.2 | ISC |
| `d3-delaunay` | 6.0.4 | ISC |
| `d3-dispatch` | 3.0.1 | ISC |
| `d3-drag` | 3.0.0 | ISC |
| `d3-dsv` | 3.0.1 | ISC |
| `d3-ease` | 3.0.1 | BSD-3-Clause |
| `d3-fetch` | 3.0.1 | ISC |
| `d3-force` | 3.0.0 | ISC |
| `d3-format` | 3.1.2 | ISC |
| `d3-geo` | 3.1.1 | ISC |
| `d3-hierarchy` | 3.1.2 | ISC |
| `d3-interpolate` | 3.0.1 | ISC |
| `d3-path` | 3.1.0 | ISC |
| `d3-polygon` | 3.0.1 | ISC |
| `d3-quadtree` | 3.0.1 | ISC |
| `d3-random` | 3.0.1 | ISC |
| `d3-sankey` | 0.12.3 | BSD-3-Clause |
| `d3-scale` | 4.0.2 | ISC |
| `d3-scale-chromatic` | 3.1.0 | ISC |
| `d3-selection` | 3.0.0 | ISC |
| `d3-shape` | 3.2.0 | ISC |
| `d3-time` | 3.1.0 | ISC |
| `d3-time-format` | 4.1.0 | ISC |
| `d3-timer` | 3.0.1 | ISC |
| `d3-transition` | 3.0.1 | ISC |
| `d3-zoom` | 3.0.0 | ISC |
| `dagre-d3-es` | 7.0.14 | MIT |
| `data-urls` | 7.0.0 | MIT |
| `dayjs` | 1.11.21 | MIT |
| `de-indent` | 1.0.2 | MIT |
| `debug` | 4.4.3 | MIT |
| `decimal.js` | 10.6.0 | MIT |
| `deep-eql` | 5.0.2 | MIT |
| `deep-is` | 0.1.4 | MIT |
| `define-data-property` | 1.1.4 | MIT |
| `define-properties` | 1.2.1 | MIT |
| `delaunator` | 5.1.0 | ISC |
| `dompurify` | 3.4.13 | (MPL-2.0 OR Apache-2.0) |
| `dunder-proto` | 1.0.1 | MIT |
| `earcut` | 3.2.3 | ISC |
| `eastasianwidth` | 0.2.0 | MIT |
| `editorconfig` | 1.0.7 | MIT |
| `emmet` | 2.4.11 | MIT |
| `emoji-regex` | 9.2.2 | MIT |
| `entities` | 4.5.0 | BSD-2-Clause |
| `env-paths` | 4.0.0 | MIT |
| `es-define-property` | 1.0.1 | MIT |
| `es-errors` | 1.3.0 | MIT |
| `es-module-lexer` | 1.7.0 | MIT |
| `es-object-atoms` | 1.1.2 | MIT |
| `es-toolkit` | 1.49.0 | MIT |
| `esbuild` | 0.28.1 | MIT |
| `eslint-scope` | 8.4.0 | BSD-2-Clause |
| `eslint-visitor-keys` | 4.2.1 | Apache-2.0 |
| `esprima` | 4.0.1 | BSD-2-Clause |
| `esrecurse` | 4.3.0 | BSD-2-Clause |
| `estraverse` | 5.3.0 | BSD-2-Clause |
| `estree-walker` | 3.0.3 | MIT |
| `esutils` | 2.0.3 | BSD-2-Clause |
| `eventemitter3` | 5.0.4 | MIT |
| `execa` | 5.1.1 | MIT |
| `expect-type` | 1.4.0 | Apache-2.0 |
| `exsolve` | 1.1.0 | MIT |
| `fast-deep-equal` | 3.1.3 | MIT |
| `fast-levenshtein` | 2.0.6 | MIT |
| `fdir` | 6.5.0 | MIT |
| `for-each` | 0.3.5 | MIT |
| `foreground-child` | 3.3.1 | ISC |
| `fsevents` | 2.3.3 | MIT |
| `function-bind` | 1.1.2 | MIT |
| `fzf` | 0.5.2 | BSD-3-Clause |
| `generator-function` | 2.0.1 | MIT |
| `get-intrinsic` | 1.3.0 | MIT |
| `get-proto` | 1.0.1 | MIT |
| `get-stream` | 6.0.1 | MIT |
| `gifuct-js` | 2.1.2 | MIT |
| `glob` | 10.5.0 | ISC |
| `gopd` | 1.2.0 | MIT |
| `gsap` | 3.15.0 | Standard 'no charge' license: https://gsap.com/standard-license. |
| `hachure-fill` | 0.5.2 | MIT |
| `has-flag` | 4.0.0 | MIT |
| `has-property-descriptors` | 1.0.2 | MIT |
| `has-symbols` | 1.1.0 | MIT |
| `has-tostringtag` | 1.0.2 | MIT |
| `hasown` | 2.0.4 | MIT |
| `he` | 1.2.0 | MIT |
| `highlight.js` | 11.11.1 | BSD-3-Clause |
| `hookable` | 5.5.3 | MIT |
| `html-encoding-sniffer` | 6.0.0 | MIT |
| `human-signals` | 2.1.0 | Apache-2.0 |
| `iconv-lite` | 0.6.3 | MIT |
| `import-meta-resolve` | 4.2.0 | MIT |
| `inherits` | 2.0.4 | ISC |
| `ini` | 1.3.8 | ISC |
| `internmap` | 2.0.3 | ISC |
| `inversify` | 7.11.0 | MIT |
| `is-arguments` | 1.2.0 | MIT |
| `is-buffer` | 2.0.5 | MIT |
| `is-callable` | 1.2.7 | MIT |
| `is-fullwidth-code-point` | 3.0.0 | MIT |
| `is-generator-function` | 1.1.2 | MIT |
| `is-nan` | 1.3.2 | MIT |
| `is-node-process` | 1.2.0 | MIT |
| `is-potential-custom-element-name` | 1.0.1 | MIT |
| `is-regex` | 1.2.1 | MIT |
| `is-safe-filename` | 0.1.1 | MIT |
| `is-stream` | 2.0.1 | MIT |
| `is-typed-array` | 1.1.15 | MIT |
| `is-what` | 5.5.0 | MIT |
| `isexe` | 2.0.0 | ISC |
| `ismobilejs` | 1.1.1 | MIT |
| `jackspeak` | 3.4.3 | BlueOak-1.0.0 |
| `javascript-obfuscator` | 5.6.0 | BSD-2-Clause |
| `jose` | 5.10.0 | MIT |
| `js-beautify` | 1.15.4 | MIT |
| `js-binary-schema-parser` | 2.0.3 | MIT |
| `js-cookie` | 3.0.8 | MIT |
| `js-string-escape` | 1.0.1 | MIT |
| `js-tokens` | 9.0.1 | MIT |
| `jsdom` | 29.1.1 | MIT |
| `jsesc` | 3.1.0 | MIT |
| `json5` | 2.2.3 | MIT |
| `katex` | 0.18.1 | MIT |
| `khroma` | 2.1.0 | Unknown |
| `layout-base` | 1.0.2 | MIT |
| `levn` | 0.3.0 | MIT |
| `libphonenumber-js` | 1.13.12 | MIT |
| `linkify-it` | 5.0.2 | MIT |
| `local-pkg` | 1.2.1 | MIT |
| `lodash-es` | 4.18.1 | MIT |
| `loupe` | 3.2.1 | MIT |
| `lru-cache` | 11.5.2 | BlueOak-1.0.0 |
| `magic-string` | 0.30.21 | MIT |
| `magic-string-ast` | 1.0.3 | MIT |
| `markdown-it` | 14.3.0 | MIT |
| `markdown-it-abbr` | 2.0.0 | MIT |
| `markdown-it-attrs` | 5.0.1 | MIT |
| `markdown-it-container` | 4.0.0 | MIT |
| `markdown-it-deflist` | 4.0.0 | MIT |
| `markdown-it-footnote` | 4.0.0 | MIT |
| `markdown-it-ins` | 4.0.0 | MIT |
| `markdown-it-mark` | 4.0.0 | MIT |
| `markdown-it-sub` | 2.0.0 | MIT |
| `markdown-it-sup` | 2.0.0 | MIT |
| `markdown-it-task-lists` | 2.1.1 | ISC |
| `marked` | 15.0.12 | MIT |
| `material-file-icons` | 2.4.0 | MIT |
| `math-intrinsics` | 1.1.0 | MIT |
| `md5` | 2.3.0 | BSD-3-Clause |
| `mdn-data` | 2.27.1 | CC0-1.0 |
| `mdurl` | 2.0.0 | MIT |
| `merge-stream` | 2.0.0 | MIT |
| `mermaid` | 11.16.1 | MIT |
| `mimic-fn` | 2.1.0 | MIT |
| `minimatch` | 9.0.9 | ISC |
| `minipass` | 7.1.3 | BlueOak-1.0.0 |
| `mitt` | 3.0.1 | MIT |
| `mlly` | 1.8.2 | MIT |
| `ms` | 2.1.3 | MIT |
| `muggle-string` | 0.4.1 | MIT |
| `multimatch` | 5.0.0 | MIT |
| `nanoid` | 3.3.18 | MIT |
| `nopt` | 7.2.1 | ISC |
| `nostics` | 1.2.0 | MIT |
| `npm-run-path` | 4.0.1 | MIT |
| `object-is` | 1.1.6 | MIT |
| `object-keys` | 1.1.1 | MIT |
| `object.assign` | 4.1.7 | MIT |
| `obug` | 2.1.3 | MIT |
| `onetime` | 5.1.2 | MIT |
| `optionator` | 0.8.3 | MIT |
| `os-paths` | 4.4.0 | MIT |
| `package-json-from-dist` | 1.0.1 | BlueOak-1.0.0 |
| `package-manager-detector` | 1.7.0 | MIT |
| `parse-svg-path` | 0.2.0 | MIT |
| `parse5` | 8.0.1 | MIT |
| `path-browserify` | 1.0.1 | MIT |
| `path-data-parser` | 0.1.0 | MIT |
| `path-key` | 3.1.1 | MIT |
| `path-scurry` | 1.11.1 | BlueOak-1.0.0 |
| `pathe` | 2.0.3 | MIT |
| `pathval` | 2.0.1 | MIT |
| `perfect-debounce` | 1.0.0 | MIT |
| `picocolors` | 1.1.1 | ISC |
| `picomatch` | 4.0.5 | MIT |
| `pinia` | 3.0.4 | MIT |
| `pixi.js` | 8.19.0 | MIT |
| `pkg-types` | 2.3.1 | MIT |
| `points-on-curve` | 0.2.0 | MIT |
| `points-on-path` | 0.2.1 | MIT |
| `possible-typed-array-names` | 1.1.0 | MIT |
| `postcss` | 8.5.26 | MIT |
| `prelude-ls` | 1.1.2 | Unknown |
| `primevue` | 4.5.5 | MIT |
| `process` | 0.11.10 | MIT |
| `proto-list` | 1.2.4 | ISC |
| `punycode` | 2.3.1 | MIT |
| `punycode.js` | 2.3.1 | MIT |
| `quansync` | 0.2.11 | MIT |
| `readdirp` | 5.0.0 | MIT |
| `reflect-metadata` | 0.2.2 | Apache-2.0 |
| `remove-accents` | 0.5.0 | MIT |
| `require-from-string` | 2.0.2 | MIT |
| `retry` | 0.13.1 | MIT |
| `rfdc` | 1.4.1 | MIT |
| `robust-predicates` | 3.0.3 | Unlicense |
| `rollup` | 4.62.2 | MIT |
| `roughjs` | 4.6.6 | MIT |
| `rw` | 1.3.3 | BSD-3-Clause |
| `safe-regex-test` | 1.1.0 | MIT |
| `safer-buffer` | 2.1.2 | MIT |
| `saxes` | 6.0.0 | ISC |
| `scule` | 1.3.0 | MIT |
| `semver` | 7.8.5 | ISC |
| `set-function-length` | 1.2.2 | MIT |
| `shebang-command` | 2.0.0 | MIT |
| `shebang-regex` | 3.0.0 | MIT |
| `siginfo` | 2.0.0 | ISC |
| `signal-exit` | 4.1.0 | ISC |
| `source-map` | 0.6.1 | BSD-3-Clause |
| `source-map-js` | 1.2.1 | BSD-3-Clause |
| `speakingurl` | 14.0.1 | BSD-3-Clause |
| `stackback` | 0.0.2 | MIT |
| `std-env` | 3.10.0 | MIT |
| `string-template` | 1.0.0 | MIT |
| `string-width` | 4.2.3 | MIT |
| `string-width` | 5.1.2 | MIT |
| `stringz` | 2.1.0 | MIT |
| `strip-ansi` | 6.0.1 | MIT |
| `strip-ansi` | 7.2.0 | MIT |
| `strip-final-newline` | 2.0.0 | MIT |
| `strip-literal` | 3.1.0 | MIT |
| `style-mod` | 4.1.3 | MIT |
| `stylis` | 4.4.0 | MIT |
| `superjson` | 2.2.6 | MIT |
| `supports-color` | 7.2.0 | MIT |
| `symbol-tree` | 3.2.4 | MIT |
| `throttleit` | 2.1.0 | MIT |
| `tiny-lru` | 11.4.7 | BSD-3-Clause |
| `tinybench` | 2.9.0 | MIT |
| `tinyexec` | 0.3.2 | MIT |
| `tinyglobby` | 0.2.17 | MIT |
| `tinypool` | 1.1.1 | MIT |
| `tinyrainbow` | 2.0.0 | MIT |
| `tinyspy` | 4.0.4 | MIT |
| `tldts` | 7.4.8 | MIT |
| `tldts-core` | 7.4.8 | MIT |
| `tough-cookie` | 6.0.2 | BSD-3-Clause |
| `tr46` | 6.0.0 | MIT |
| `ts-dedent` | 2.3.0 | MIT |
| `tslib` | 2.8.1 | 0BSD |
| `type-check` | 0.3.2 | MIT |
| `typescript` | 5.9.3 | Apache-2.0 |
| `uc.micro` | 2.1.0 | MIT |
| `ufo` | 1.6.4 | MIT |
| `undici` | 7.29.0 | MIT |
| `undici-types` | 8.3.0 | MIT |
| `unplugin` | 3.3.0 | MIT |
| `unplugin-utils` | 0.3.2 | MIT |
| `unplugin-vue-components` | 32.1.0 | MIT |
| `util` | 0.12.5 | MIT |
| `uuid` | 14.0.1 | MIT |
| `validator` | 13.15.35 | MIT |
| `vite` | 7.3.6 | MIT |
| `vite-node` | 3.2.4 | MIT |
| `vitest` | 3.2.6 | MIT |
| `vscode-jsonrpc` | 9.0.1 | MIT |
| `vscode-languageserver-protocol` | 3.18.2 | MIT |
| `vscode-languageserver-types` | 3.18.0 | MIT |
| `vscode-uri` | 3.1.0 | MIT |
| `vue` | 3.5.40 | MIT |
| `vue-component-type-helpers` | 3.3.7 | MIT |
| `vue-demi` | 0.14.10 | MIT |
| `vue-router` | 5.2.0 | MIT |
| `vue-tsc` | 2.2.12 | MIT |
| `w3c-keyname` | 2.2.8 | MIT |
| `w3c-xmlserializer` | 5.0.0 | MIT |
| `webidl-conversions` | 8.0.1 | BSD-2-Clause |
| `webpack-virtual-modules` | 0.6.2 | MIT |
| `whatwg-mimetype` | 5.0.0 | MIT |
| `whatwg-url` | 16.0.1 | MIT |
| `which` | 2.0.2 | ISC |
| `which-typed-array` | 1.1.22 | MIT |
| `why-is-node-running` | 2.3.0 | MIT |
| `word-wrap` | 1.2.5 | MIT |
| `wrap-ansi` | 7.0.0 | MIT |
| `wrap-ansi` | 8.1.0 | MIT |
| `xdg-app-paths` | 5.5.1 | MIT |
| `xdg-portable` | 7.3.0 | MIT |
| `xml-name-validator` | 5.0.0 | Apache-2.0 |
| `xmlchars` | 2.2.0 | MIT |
| `yaml` | 2.9.0 | ISC |
| `zod` | 4.1.11 | MIT |
